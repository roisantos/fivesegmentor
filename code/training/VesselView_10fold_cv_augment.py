...
class FIVESJoinedDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Apply augment_otrosfives augmentation
        if random.random() < 0.5:  # 50% chance of applying geometric transformations
            # Random rotation with fixed angles
            allowed_angles = [-180, -90, 0, 90]
            angle = random.choice(allowed_angles)
            center = (image_np.shape[1] // 2, image_np.shape[0] // 2)
            rot_mat = cv2.getRotationMatrix2D(center, angle, 1.0)
            image_np = cv2.warpAffine(image_np, rot_mat, (image_np.shape[1], image_np.shape[0]), flags=cv2.INTER_LINEAR)
            label_np = cv2.warpAffine(label_np, rot_mat, (label_np.shape[1], label_np.shape[0]), flags=cv2.INTER_NEAREST)
            
            # Random horizontal flip
            if random.random() < 0.5:
                image_np = cv2.flip(image_np, 1)W
                label_np = cv2.flip(label_np, 1)
            
            # Random vertical flip
            if random.random() < 0.5:
                image_np = cv2.flip(image_np, 0)
                label_np = cv2.flip(label_np, 0)
